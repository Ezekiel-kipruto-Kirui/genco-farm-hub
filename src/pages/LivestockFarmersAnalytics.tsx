...
const LivestockFarmersAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "Livestock Farmers"));
      ...
      data.forEach(farmer => {
        totalMaleGoats += parseInt(farmer.goatsMale || farmer.GoatsMale || 0);
        totalFemaleGoats += parseInt(farmer.goatsFemale || farmer.GoatsFemale || 0);
      });
      setGoatsData([
        { name: "Male Goats", value: totalMaleGoats },
        { name: "Female Goats", value: totalFemaleGoats },
      ]);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold font-display mb-2">Livestock Farmers Analytics</h2>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="font-display">Goats Distribution</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
