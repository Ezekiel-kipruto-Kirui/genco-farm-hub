...
const CapacityBuildingAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "Capacity Building"));
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold font-display mb-2">Capacity Building Analytics</h2>
        <p className="text-muted-foreground">Training programs insights and statistics</p>
      </div>

      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Total Trained</CardTitle>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={regionData}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="value" fill="hsl(217 91% 60%)" name="Participants" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
...
export default CapacityBuildingAnalytics;
